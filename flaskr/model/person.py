...
class PersonModel:
    ...
    @staticmethod
    def create(name, person_type, code, address, email, phone_number):
        """
        ...
        """
        if name == "" or person_type == "" or code == "" or address == "" or email == "" or phone_number == "":
            return False

        arg = "INSERT INTO pessoas (nome, tipo, codigo, endereco, email, telefone) VALUES (%s, %s, %s, %s, %s, %s);"
        res = db_execute(arg, name, person_type, code, address, email, phone_number, fetch_type="all")
        if not res[0]:
            print(res[1])
            return False
        return True
